...
- [x] Deploy **Frontend** lên Railway / Vercel.
- [x] Deploy **Backend** lên Railway.
- [x] Cấu hình **Railway PostgreSQL** service (thay thế Supabase để tối ưu độ trễ).
- [x] Thiết lập **CI/CD** hoàn chỉnh với Docker Build Checks & Caching.
- [x] Cấu hình bảo mật Swagger (Vô hiệu hóa mặc định trên Production).

### 22. Monitoring & Logging

- [x] Tích hợp **Structured Logging** (JSON format) cho Railway Logs.
- [x] Health check endpoint (`/actuator/health`) cho uptime monitoring.
- [x] Thiết lập **Error Tracking** với Sentry.
...
| **Database**    | 5432 | 🟢 Running (Railway/Docker) |

---

_Cập nhật lần cuối: 2026-03-03_
