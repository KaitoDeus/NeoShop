...
import { 
  FiSearch, FiChevronRight, FiChevronDown, FiHelpCircle, 
  FiKey, FiRefreshCcw, FiCreditCard, FiShield, FiMessageCircle,
    ...
    <div className="help-center-page">
      <div className="container">
        ...
        <div className="breadcrumb">
          ...
          <span style={{ color: '#0f172a', fontWeight: '500' }}>Trung tâm trợ giúp</span>
        </div>
        ...
        <div className="quick-links">
          <Link to="/huong-dan-kich-hoat" className="quick-link-card">
            <FiKey className="ql-icon" />
            <span>Hướng dẫn kích hoạt</span>
            <FiChevronRight />
          </Link>
          <Link to="/chinh-sach-bao-hanh" className="quick-link-card">
            <FiRefreshCcw className="ql-icon" />
            <span>Chính sách bảo hành</span>
            <FiChevronRight />
          </Link>
          <Link to="/lien-he" className="quick-link-card">
        ...
        <div className="faq-section">
          <h2><FiHelpCircle /> Câu hỏi thường gặp</h2>
          
          <div className="faq-container">
            {FAQ_CATEGORIES.map((cat) => (
              <div key={cat.id} className={`faq-category ${openCategory === cat.id ? 'open' : ''}`}>
                <div className="faq-cat-header" onClick={() => toggleCategory(cat.id)}>
                  <div className="faq-cat-title">
                    {cat.icon}
                    <span>{cat.title}</span>
                  </div>
                  <FiChevronDown className={`chevron ${openCategory === cat.id ? 'rotate' : ''}`} />
                </div>
                  ...
                  <div className="faq-questions">
                    {cat.questions.map((item, idx) => (
                      <div key={idx} className="faq-item">
                        ...
                        >
                          <span>{item.q}</span>
                          <FiChevronRight className={`chevron ${openQuestion === `${cat.id}-${idx}` ? 'rotate' : ''}`} />
                        </div>
