...
const Orders = () => {
    ...
    switch (status) {
      ...
      case "success":
        return (
          <span className="status-badge status-active">
            <span className="status-dot"></span>Thành công
          </span>
        );
      case "PENDING":
      case "pending":
        return (
          ...
          >
            <span className="status-dot"></span>Chờ thanh toán
          </span>
        );
      case "FAILED":
      case "CANCELLED":
      case "failed":
        return (
          ...
          >
            <span className="status-dot"></span>Thất bại
          </span>
        );
      default:
  ...
  const getPaymentIcon = (method) => {
    switch (method) {
      case "MOMO":
        return <FiSmartphone className="text-purple" />;
      case "BANK_TRANSFER":
        return <FiBriefcase className="text-blue" />;
      case "VISA":
        return <FiCreditCard className="text-green" />;
      default:
        return <FiCreditCard />;
    }
  };
  ...
  const handleBulkDelete = async () => {
    if (
      window.confirm(
        `Bạn có chắc muốn xóa ${selectedIds.length} đơn hàng đã chọn?`,
      )
    ) {
      try {
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            Theo dõi, kiểm tra và xử lý các đơn hàng sản phẩm số
          </p>
        </div>
        <div
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: "0.5rem" }}>
                    <button
                      ...
                      onClick={() => {
                        const start =
                          document.getElementById("temp-start-date").value;
                        const end =
                          document.getElementById("temp-end-date").value;
                        setStartDate(start);
                        setEndDate(end);
                        setPage(0);
        ...
        {selectedIds.length > 0 && (
          <div className="bulk-actions-bar">
            <span className="selected-count">
              Đã chọn <strong>{selectedIds.length}</strong> đơn hàng
            </span>
        ...
        <div className="table-footer">
          <div className="table-info">
            ...
            <span className="font-bold">
              {orders.length > 0 ? page * 10 + 1 : 0}
            </span>{" "}
            đến{" "}
            <span className="font-bold">
              {Math.min((page + 1) * 10, totalElements)}
            </span>{" "}
            của <span className="font-bold">{totalElements}</span> đơn hàng
          </div>
