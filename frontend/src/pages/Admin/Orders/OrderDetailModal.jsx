...
const OrderDetailModal = ({ order, onClose }) => {
  ...
  const getPaymentIcon = (method) => {
    switch (method) {
      case "MOMO":
        return (
          <>
            <FiSmartphone className="text-purple" /> MOMO
          </>
        );
      case "BANK_TRANSFER":
        return (
          <>
            <FiBriefcase className="text-blue" /> Chuyển khoản
          </>
        );
      case "VISA":
        return (
          <>
            <FiCreditCard className="text-green" /> Thẻ VISA
          </>
        );
      default:
        return (
          <>
            <FiCreditCard /> {method || "Chưa xác định"}
          </>
        );
    }
  };
  ...
  const handleUpdateStatus = async (status) => {
    if (
      window.confirm(
        `Bạn có chắc muốn chuyển trạng thái đơn hàng sang ${status}?`,
      )
    ) {
      try {
          ...
          <div className="detail-grid">
            <div className="detail-box">
              ...
              <div className="box-content">
                ...
                {order.userEmail && (
                  <p>
                    <FiMail className="box-icon" /> {order.userEmail}
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="detail-box">
              <h3>Thanh toán</h3>
              <div className="box-content">
                <p className="payment-method-disp">
                ...
                {order.items && order.items.length > 0 ? (
                  order.items.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td className="item-qty" style={{ textAlign: "center" }}>
                        x{item.quantity}
                      </td>
                    </tr>
                  ))
