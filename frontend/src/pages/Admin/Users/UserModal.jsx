...
const UserModal = ({ user, onClose, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Có lỗi xảy ra khi lưu thông tin người dùng.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-content user-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '450px' }}>
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            {error && <div className="error-message" style={{ color: 'red', marginBottom: '1rem', background: '#fee2e2', padding: '0.5rem', borderRadius: '4px' }}>{error}</div>}
            
            <div className="form-group">
              <label><FiUser style={{marginBottom: -2, marginRight: 4}}/> Tên đăng nhập *</label>
              <input 
                type="text" 
                name="username" 
            ...
            {!user && (
              <div className="form-group">
                <label><FiLock style={{marginBottom: -2, marginRight: 4}}/> Mật khẩu *</label>
                <input 
                  type="password" 
                  name="password" 
          ...
          <div className="modal-footer">
            ...
            <button type="submit" className="btn-primary" disabled={loading}>
              <FiSave /> {loading ? 'Đang lưu...' : (user ? 'Cập nhật' : 'Lưu khách hàng')}
            </button>
          </div>
        </form>
      </div>
