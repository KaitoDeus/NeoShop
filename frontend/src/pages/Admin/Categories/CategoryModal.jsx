...
const CategoryModal = ({ category, categories, onClose, onSave }) => {
    ...
    setFormData(prev => {
      ...
      if (name === 'name' && (!category || !prev.slug)) {
        newData.slug = value.toLowerCase()
          .replace(/ /g, '-')
          .replace(/[vnàáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđ]/gi, (c) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(error);
      const errorMsg = error.response?.data?.message || "Lỗi khi lưu danh mục. Vui lòng kiểm tra lại slug (phải là duy nhất).";
      alert(errorMsg);
    } finally {
      setLoading(false);
    }
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            ...
            <div className="form-group">
              ...
              />
              <p className="helper-text">Hệ thống tự động tạo slug từ tên nếu để trống.</p>
            </div>
          ...
          <div className="modal-footer">
            <button type="button" className="btn-outline" onClick={onClose} style={{ padding: '0.6rem 1.2rem' }}>
              Hủy bỏ
            </button>
            <button type="submit" className="btn-primary" disabled={loading} style={{ padding: '0.6rem 1.5rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
